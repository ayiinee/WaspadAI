...
Versi 3.0.0-draft. Status PROPOSED schema Product; belum migration yang dijalankan. Dokumen menjabarkan desain hingga kolom, constraint, index, RLS dan lifecycle. SQL ilustratif harus diterapkan melalui migration dan diuji pada Supabase versi yang dipakai tim. Arsip database lama utuh di akhir bersifat NON-NORMATIVE.
...
Status Product menggunakan `text CHECK (...)` untuk kemudahan evolusi; enum AI mengikuti pinned schema dan divalidasi aplikasi. Constraint exact enum AI baru dibekukan ketika snapshot asli tersedia. Audit/projection tetap menyimpan `schema_version`.
