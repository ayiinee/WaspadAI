...
Versi 3.0.1-draft. Status TARGET. Semua tree di sini adalah rancangan file yang perlu diimplementasikan, bukan daftar source yang sudah ada dalam ZIP dokumentasi. [ADR](../adr/0001-product-ai-boundary.md) menentukan batas layanan; [database](database-architecture.md) menentukan schema.
...
│   ├── upstream/
│   │   ├── contract-lock.json
│   │   └── waspadai-ai.openapi.json     # Snapshot unggahan asli disertakan; full commit belum diketahui
│   └── examples/
├── supabase/
│   ├── config.toml
│   ├── migrations/
